...
export function getPossibleStatAwakeParams(otherOption, itemLevel, i18n) {
    ...
    const res = { "none": i18n.t("stat_awake_none") };
    for (const awake of statAwakes) {
        ...
        if (!Object.keys(res).includes(p)) {
            res[p] = getStatNameByIdOrDefault(p, i18n);
